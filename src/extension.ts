...
function getThemeCss(): string {
  ...
  try {
    ...
    if (match) { return fs.readFileSync(path.join(themesDir, match.name, 'theme.css'), 'utf-8'); }
  } catch { /* .obsidian/themes itself missing or unreadable */ }

  vscode.window.showWarningMessage(
    `Vault Tool: no se encontró el tema "${themeName}" en .obsidian/themes — revisa mayúsculas/minúsculas del nombre.`
  );
  return '';
}
