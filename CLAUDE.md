...
| `toggle-task-at-location` / `edit-task-at-location` | `{ path, line }` variants of `toggle-task`/`edit-task` for a task that may live in a *different* file than the one this panel is showing — used by ```tasks``` query rows. See "Task checkboxes" and "```tasks``` query blocks" below. |
| `edit-task` | Opens the Tasks extension's "Create or edit Task" dialog for the task at `msg.line` (0-based) in the file this panel is showing. See "Task checkboxes" below. |
...
interface TasksExtensionApi {
  ...
  editTaskAtLocation?(path: string, line: number): Promise<void>; // opens that extension's own edit dialog; see "Editing a task" below
  onDidChangeTasks?: vscode.Event<void>;
}
```

#### Editing a task (pencil icon, `edit-task` / `edit-task-at-location`)

There is no cursor-based "edit task" command on this side, unlike the Tasks extension's own `tasksManager.createOrEditTask` — this editor is a `CustomTextEditorProvider`, so VS Code doesn't expose anything like `activeTextEditor.selection` for it, and the Tasks extension can't read cursor position out of our webview either (same isolation as everywhere else in this integration). Concretely: before this existed, running the Tasks extension's own command palette action while a note was open here silently created a *new*, blank task appended to the end of the document instead of editing the one the user meant, because that extension genuinely had no way to know which line was "current."

The fix is symmetrical to how toggling already avoids this problem: a small pencil button (`.cm-task-edit-btn`, rendered next to every checkbox by `TaskCheckboxWidget` and every `renderTaskRow` — see below) posts `{ type: 'edit-task', line }` (current document) or `{ type: 'edit-task-at-location', path, line }` (a ```tasks``` query row, possibly a different file), giving the Tasks extension the exact location up front instead of asking it to infer one. The host handler resolves `path` via `vscode.workspace.asRelativePath(document.uri, false)` for the plain `edit-task` case, then calls `tasksApi.editTaskAtLocation(path, line)`, which opens that extension's own "Create or edit Task" `WebviewPanel`, pre-filled from the task at that location, and applies the result via `WorkspaceEdit` once the user clicks Apply. If `editTaskAtLocation` isn't available (extension missing or an older build without it), shows an informational message rather than silently doing nothing — there's no local fallback implementation for this one (unlike `naiveToggleTaskLine`), since re-implementing the dialog here would defeat the point of delegating to the sibling extension in the first place.
...
- **`Link` nodes shaped like a `[[wiki-link]]`'s inner bracket pair, active line only** — marked `cm-wiki-link-raw`; see below.
...
**`[[wiki-link]]` inner-bracket coloring while editing (`.cm-wiki-link-raw`)**: `[[Foo]]` isn't real CommonMark syntax, but lezer-markdown parses its *inner* `[Foo]` as an ordinary shortcut-reference `Link` node (with its own `LinkMark` children) purely because it's indistinguishable from single-bracket link syntax — confirmed by dumping the syntax tree for `[[Foo]]`, which produces `Link[7,12]` plus two `LinkMark` children, while the *outer* `[` and `]` fall completely outside any node. Since `mdHighlight` colors `tags.link`/`tags.processingInstruction` unconditionally (CM6's own `syntaxHighlighting()`, independent of and always active regardless of what `livePreviewPlugin`/`wikiLinkPlugin` decide to hide), the inner brackets ended up colored + smaller font-size while the outer ones stayed plain text color — a visible mismatch while actively editing the line (non-active lines never showed this, since `wikiLinkPlugin`'s own decorations already replace/hide the whole `[[...]]` span). Fixed by detecting, **on the active line only**, a `Link` node immediately preceded by `[` and immediately followed by `]` (the signature of being the inner pair of a doubled bracket) and marking it `cm-wiki-link-raw`, whose `vsTheme` rule resets `color`/`textDecoration`/`fontSize` to `inherit`/`none`/`inherit` with `!important` (needed since `mdHighlight`'s generated classes aren't `!important` themselves, but load order between the two stylesheets isn't guaranteed). Two things this deliberately does not touch: `![[Foo]]` (a transclusion) — the leading `!` makes the *outer* `Image` node wrap everything, so every character including its brackets already gets uniform `tags.link` styling with nothing to fix; detected via `node.node.parent.name !== 'Image'`. And it's gated to the active line specifically — an earlier version applied it unconditionally, which also decorated the *non-active* rendering, where `wikiLinkPlugin`'s own decorations already occupy the same node range; the `!important` reset then cancelled `wikiLinkPlugin`'s `cm-wiki-link` blue/underline styling on the just-resolved link text instead of leaving it alone.
...
- The `<marker> [<char>] ` span is replaced with `TaskCheckboxWidget(checked, line0based)` — a `WidgetType` whose `toDOM()` returns a `<span class="cm-task-widget">` wrapping *two* children: the real `<input type="checkbox">` (class `cm-task-checkbox`, `data-line` set to the 0-based doc line number) and a `<span class="cm-task-edit-btn">✏️</span>` (same `data-line`) — see "Editing a task" above. **Unlike `BulletWidget`, this is rendered on the active/cursor line too** (the task-line branch runs before the `active.has(ln)` gate that hides other inline markers on the cursor's line), so the checkbox and edit button stay clickable while editing that line's text.
...
- Clicking `.cm-task-checkbox` or `.cm-task-edit-btn` is wired into the existing `linkClickHandler` (`mousedown` prevents default / `click` fires the action, same pattern as wiki-links and `cm-md-link`): the checkbox sends `vscode.postMessage({ type: 'toggle-task', line: Number(el.dataset.line) })`; the pencil button sends `{ type: 'edit-task', line }`. The extension host applies the actual toggle/opens the edit dialog (see "Task checkboxes" under extension-host message handlers above) and the resulting document change flows back through the normal `external-update` path, so the widget re-renders with the new state once applied — there is no local optimistic DOM update.
...
- `renderTasksQueryResult(container, result)` builds real DOM (`textContent`, not `innerHTML` — task descriptions are never interpreted as HTML) via `renderTaskRow`/`renderTaskList`: one `<div class="cm-tasks-query-item">` per task, containing a checkbox (class `cm-task-checkbox cm-task-query-checkbox` — **both** classes, so it still picks up `vsTheme`'s checkbox styling), priority/due-date/recurrence badges (`cm-tasks-query-badge`, overdue ones also get `cm-task-overdue`), and a trailing edit button (class `cm-task-edit-btn cm-task-query-edit-btn`, same dual-class trick). `result.groups` (when the query has `group by`) renders a `cm-tasks-query-group-title` heading per non-empty group instead of one flat list. `result.unrecognizedLines` (query lines the engine didn't understand) are surfaced as a visible `cm-tasks-query-warning`, never silently dropped.
- A tasks-query checkbox/edit-button carries `data-path` **and** `data-line` (a result can come from any file in the vault, not just the open one), so `linkClickHandler` checks `.cm-task-query-checkbox` *before* the plain `.cm-task-checkbox` branch (and `.cm-task-query-edit-btn` before `.cm-task-edit-btn`, same reason — the query variants match both selectors) and sends `{ type: 'toggle-task-at-location', path, line }` / `{ type: 'edit-task-at-location', path, line }` instead of the single-file `{ type: 'toggle-task', line }` / `{ type: 'edit-task', line }`.
