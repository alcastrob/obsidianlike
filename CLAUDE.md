...
- `mdHighlight` gives `tags.monospace` a stable class (`cm-inline-code`) instead of inline CSS properties (same `class`-only pattern as headings). `vsTheme`'s `.cm-inline-code` rule has the chip styling.
- `livePreviewPlugin`'s `FencedCode` handling (the "not a `tasks` block" fall-through path) adds a `Decoration.line()` class to **every** line of the block (fence-open, content, fence-close) — `cm-code-block` on all of them, plus `cm-code-block-first`/`cm-code-block-last` (or `cm-code-block-solo` for a single-line block) for the outer rounded corners/border — unconditionally, active or not, same as heading line classes, so the box stays visible while editing inside it (only the ` ``` ` fence marks reveal/hide per the usual active-line check). `vsTheme` then has a plain descendant-selector override, `.cm-code-block .cm-inline-code { background: none; padding: 0; borderRadius: 0; }`, which cancels the chip look specifically for `CodeText` found inside a code-block line — ordinary CSS specificity (two classes beat one), no fighting over decoration nesting order needed.
